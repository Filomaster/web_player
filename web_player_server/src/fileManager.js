...
module.exports = {
  ...
  getSongs: function (album) {
    return new Promise((resolve, reject) => {
      let songs = [];
      this.readDir(path.join(__dirname, "../static/mp3", album))
        .then((files) => {
          files.forEach((file) => {
            if (file.endsWith(".mp3")) {
              songs.push({
                file: encodeURI(file),
                size: fs.statSync(
                  path.join(__dirname, "../static/mp3", album, file)
                ).size,
              });
            }
          });
      ...
      for (let j = 0; j < dirs.length; j++) {
        promises.push(
          new Promise((resolve, reject) => {
            this.readDir(path.join(__dirname, "../static/mp3", dirs[j]))
              .then((files) => {
  ...
  getMusicList: function (album = null) {
    return new Promise((resolve, reject) => {
      ...
      this.readDir(path.join(__dirname, "../static/mp3"))
        .then((dirs) => {
          fileList.dirs = dirs;
