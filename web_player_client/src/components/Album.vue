<template>
  <div
    class="album"
    :style="{
      backgroundImage: `url('http://localhost:3000/images/?file=${cover}')`,
    }"
    :class="{ selected: id == getAlbumSelection() }"
    v-on:click.stop="setSelection(id)"
  >
    <div class="text">{{ decodeTitle(title) }}</div>
    <div v-if="id == getAlbumSelection(id)" class="overlay">
...
.text {
  ...
  background: linear-gradient(
    0deg,
    rgba(0, 0, 0, 0.8071429255295868) 0%,
    rgba(0, 0, 0, 0.10126057258841037) 100%
  );
}
